...
import { Plus, Edit, ArrowUpRight, Search, Layout, Calendar, Tag, CheckCircle2, Circle } from 'lucide-react';
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-100">
                            <tr>
                                <th className="px-6 py-4 font-semibold text-xs uppercase tracking-wider text-slate-500">Template</th>
                                    ...
                                    <td colSpan={5} className="px-6 py-20 text-center text-slate-500">
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm mt-1">Get started by creating your first template.</p>
                                            </div>
                                            <Link href="/admin/templates/new" className="mt-2 text-primary hover:underline font-medium">Create Template</Link>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                templates.map((template) => (
                                    <tr key={template.id} className="group hover:bg-slate-50/80 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="font-semibold text-slate-900 text-base">{template.name.id}</span>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${template.isPublished === 'true'
                                                ...
                                                {template.isPublished === 'true' ? 'Published' : 'Draft'}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                                                <Link
                                                    href={`/id/library/${template.category}/${template.slug}`}
                                                    target="_blank"
